    ...
    if(histograms.isOpened()) {
        ...
        if(histograms["cam_angle"].isInt() ||
           histograms["cam_angle"].isReal()) {
            ...
            tanCamAngle = tan(camAngle*3.14159/180.0);
        }
    } else {
        gain1 = 2.5;
        gain2 = 0.1;
    ...
    while(true) {
        ...
        if (
            ...
            if(i >= 0) {
                ...
                std::chrono::duration<float> _dt = std::chrono::duration_cast<std::chrono::duration<float>>(now - last);
                last = now;
                float dt = _dt.count();
                ...
                ball.y = ((float)(tanCamAngle + (centroid.y - cy)/fy)) / (1 - tanCamAngle*(centroid.y - cy)/fy);
